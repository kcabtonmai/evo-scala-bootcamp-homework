...
object HomeworkSpec {
  ...
  //implicit val decodeDate: Decoder[LocalDate] = Decoder.decodeLocalDate.map(input => LocalDate.parse(input.toString, DateTimeFormatter.ISO_DATE))

  implicit val decodeDate: Decoder[LocalDate] = Decoder.decodeString.map {
    input => LocalDate.parse(input, DateTimeFormatter.ofPattern("yyyyMMdd"))
  }

  implicit val decodeTeamTotals: Decoder[TeamTotals] = Decoder.instance{ h =>
    for{
      assists               <-  h.get[String]("assists")
      fullTimeoutRemaining  <-  h.get[String]("full_timeout_remaining")
      plusMinus             <-  h.get[String]("plusMinus")
