...
object DataStructuresHomework {
  ...
  def sortConsideringEqualValues[T](map: Map[T, Int]): List[(Set[T], Int)] = {
    (for {
      k <- map.groupBy { case (_, value) => value }.keySet
      v <- map.groupBy { case (_, value) => value }.get(k)
    }
      yield v.toList.sortBy { case (key, _) => key.asInstanceOf[String] }.toMap.keys.toSet -> k)
      .toList.sortWith { case (_, (_,value)) => value < value }
  }
