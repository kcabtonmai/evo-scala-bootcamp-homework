...
object ImplicitsHomework {
  ...
  object SuperVipCollections4s {
    ...
    final class MutableBoundedCache[K: GetSizeScore, V: GetSizeScore](maxSizeScore: SizeScore) {
      ...
      def pop(): Unit = if(map.nonEmpty) map.remove(map.head._1)

      def put(key: K, value: V): Unit = {
        while(map.sizeScore + key.sizeScore + value.sizeScore > maxSizeScore && map.nonEmpty) pop()
        if(map.sizeScore + key.sizeScore + value.sizeScore <= maxSizeScore) map.put(key, value)
        else throw new Exception("Item is too large for this map")
      }
    ...
    object instances {
      ...
      implicit def stringGetSizeScore: GetSizeScore[String] = (x: String) => 12 + (x.length * 2)
      implicit def arrayGetSizeScore[T: GetSizeScore]: GetSizeScore[Array[T]] = x => 12 + x.map(_.sizeScore).sum
      implicit def listGetSizeScore[T: GetSizeScore]: GetSizeScore[List[T]] = x => 12 + x.map(_.sizeScore).sum
      implicit def vectorGetSizeScore[T: GetSizeScore] : GetSizeScore[Vector[T]] = x => 12 + x.map(_.sizeScore).sum

      implicit def mapGetSizeScore[T: GetSizeScore, S: GetSizeScore]: GetSizeScore[Map[T, S]] =
        (x: Map[T, S]) => 12 + x.map { case (x, s) => x.sizeScore + s.sizeScore }.sum

      implicit def packedMapGetSizeScore[T: GetSizeScore, S: GetSizeScore]: GetSizeScore[PackedMultiMap[T, S]] =
        (x: PackedMultiMap[T, S]) => 12 + x.inner.map { case (x, s) => x.sizeScore + s.sizeScore }.sum

      implicit def linkedHashMapGetSizeScore[T: GetSizeScore, S: GetSizeScore]: GetSizeScore[LinkedHashMap[T, S]] =
        (x: LinkedHashMap[T, S]) => x.map { case (x, s) => x.sizeScore + s.sizeScore }.sum
  ...
  object MyTwitter {
    ...
    def createTwitCache(maxSizeScore: SizeScore): TwitCache = new TwitCache {
      ...
      implicit val scoreTwitGetSizeScore: GetSizeScore[Twit] = (x: Twit) =>
        x.id.sizeScore + x.userId.sizeScore + x.hashTags.sizeScore + x.fbiNotes.sizeScore + x.attributes.sizeScore
